...
class RRectPainter extends CustomPainter {
  ...
  const RRectPainter(
      {Key? key, this.showDetails = false, required this.withNotification});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showDetails) {
      path.addRect(
        const Rect.fromLTWH(0, 30, 93, 40),
      );
      double degToRad(num deg) => deg * (pi / 180.0);
      pathCorn.arcTo(const Rect.fromLTWH(-25, 34, 30, 30), degToRad(0),
          degToRad(90), true);
      pathCorn.arcTo(const Rect.fromLTWH(88, 34, 30, 30), degToRad(90),
          degToRad(90), true);
    }
